...
MEMORY_PATH      = DataPath("memory") / "long_term.json"
_lock            = Lock()
...
MEMORY_KEYWORDS = [
    "mein name", "ich heiße", "ich bin", "bin ich", "heiße",
    "ich wohne", "meine stadt", "mein ort",
    "ich arbeite", "mein job", "mein beruf",
    "mein geburtstag", "geboren",
    "mein lieblings", "mag ich", "ich mag", "ich liebe", "gefällt mir",
    "mein projekt", "ich baue", "ich entwickle", "ich programmiere",
    "mein freund", "meine freundin", "mein bruder", "meine schwester",
    "mein mann", "meine frau", "mein partner",
    "ich möchte", "ich will", "ich wünsche", "mein ziel", "mein traum",
    "ich hasse", "ich mag nicht",
    "meine hobbie", "in meiner freizeit",
]

def should_extract_memory(user_text: str, jarvis_text: str, api_key: str = "") -> bool:
    text = (user_text + " " + jarvis_text).lower()
    return any(kw in text for kw in MEMORY_KEYWORDS)


def extract_memory(user_text: str, jarvis_text: str, api_key: str = "") -> dict:
    # Auto-Extraktion deaktiviert — nur noch keyword-basiert (save_memory Tool)
    return {}
